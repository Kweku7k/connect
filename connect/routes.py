...
@connect.route('/broadcast', methods=['GET', 'POST'])
@connect.route('/broadcast/<int:groupId>', methods=['GET', 'POST'])
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            ...
            if current_user.credits < 0:
                ...
                return redirect(url_for('connect.purchase'))
...
@connect.route('/broadcastemail', methods=['GET', 'POST'])
@connect.route('/broadcastemail/<int:groupId>', methods=['GET', 'POST'])
...
@connect.route('/purchase', methods=['GET', 'POST'])
def purchase():
    current_user = get_current_user()
    form = TakePayment()
...
@connect.route('/groups/<string:appId>', methods=['GET', 'POST'])
def getGroups(appId):
    data = getgroup(appId)
...
@connect.route('/groups', methods=['GET', 'POST'])
def groups():   
    current_user = get_current_user()
...
@connect.route('/adduser', methods=['GET', 'POST'])
def adduser():
    form = AddUserForm()
    return render_template('adduser.html', form=form)


@connect.route('/recipt', methods=['GET', 'POST'])
def recipt():
    return render_template('email/receipt.html')

@connect.route('/booking', methods=['GET', 'POST'])
def booking():
    return render_template('email/booking.html')

@connect.route('/cureset', methods=['GET', 'POST'])
def cureset():
    return render_template('email/cureset.html')

@connect.route('/newsletter', methods=['GET', 'POST'])
def newsletter():
    # news_items = [
    #     {
...
def group(groupId):
    if request.method == 'POST':
        ...
        save_path = os.path.join(Config.root_path,"Uploads", filename)  # Replace "path_to_save" with your desired path
...
@connect.route('/delete/<int:id>', methods=['POST'])
def delete_contact(id):
    return redirect('contacts', )

@connect.route('/deleteGroup/<int:id>', methods=['GET', 'POST'])
def deleteGroup(id):
...
@connect.route('/report/<int:id>', methods=['GET', 'POST'])
def report(id):
    current_user = get_current_user()
...
@connect.route('/update/<int:id>', methods=['GET', 'POST'])
...
@connect.route('/new', methods=['GET', 'POST'])
@connect.route('/new/<int:groupId>', methods=['GET', 'POST'])
@token_required
...
@connect.route('/onboard', methods=['GET', 'POST'])
def onboard():
    form = RegisterForm()
    if form.validate_on_submit():
...
@connect.route('/reset', methods=['GET', 'POST'])
def reset():
    return render_template('email/reset.html')

@connect.route('/thankyou', methods=['GET', 'POST'])
def thankyou():
    return render_template('email/thankyou.html')

@connect.route('/election')
def election():
...
@connect.route('/dynamic_csv', methods=['GET', 'POST'])
def dynamic_csv():
    # accept csv file
    if request.method == 'POST':
...
@connect.route('/fetch-metadata', methods=['POST'])
